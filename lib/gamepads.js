...
Gamepads.prototype.observeButtonChanges = function (gamepad) {
  ...
  currentPad.buttons.forEach(function (button, buttonIdx) {
    if (button.value !== previousPad.buttons[buttonIdx].value) {
      ...
      self.fireKeyEvent(currentPad, buttonIdx, button.value);
    }
  });
};
...
Gamepads.prototype.fireKeyEvent = function (gamepad, button, value) {
  var self = this;

  if (!self.keyEventsEnabled) {
    return;
  }

  var buttonName = swap(gamepad.indices)[button];

  var names = value === 1 ? ['keydown', 'keypress'] : ['keyup'];
  var data = self.keyEvents[buttonName];
  ...
  if (data) {
    ...
    names.forEach(function (name) {
      Gamepads.utils.triggerEvent(data.target || document.activeElement, name, data);
    });
  }
};
...
Gamepads.prototype.buttonEvent2axisEvent = function (e) {
...
var clone = Gamepads.utils.clone = function (obj) {
...
var swap = Gamepads.utils.swap = function (obj) {
  var ret = {};
  for (var attr in obj) {
    if (attr in obj) {
      ret[obj[attr]] = attr;
